...
module('Integration | Component | selected-billing-plan', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    assert.equal(profilePage.billing.selectedPlanOverview.users.text, `Up to ${this.plan1.startingUsers} unique users User licenses incl. in price: User licenses at discounted credits price: check pricing Users charged monthly per usage check pricing`);

...
module('Integration | Component | repo actions', function (hooks) {
  ...
  test('it shows cancel button only if job is cancelable', async function (assert) {
    ...
    await render(hbs`{{repo-actions userHasCancelPermissionForRepo=false job=this.job}}`);
    assert.dom('button[aria-label="Cancel job"]').doesNotExist();

    await render(hbs`{{repo-actions userHasCancelPermissionForRepo=true job=this.job}}`);
  ...
  test('it shows cancel button for build only if build is cancelable', async function (assert) {
    ...
    await render(hbs`{{repo-actions userHasCancelPermissionForRepo=false build=this.build}}`);
    assert.dom('button[aria-label="Cancel build"]').doesNotExist();

    await render(hbs`{{repo-actions userHasCancelPermissionForRepo=true build=this.build}}`);
  ...
  test('it shows restart button only if job is restartable', async function (assert) {
    ...
    await render(hbs`{{repo-actions userHasRestartPermissionForRepo=false job=this.job}}`);
    assert.dom('button[aria-label="Restart job"]').doesNotExist();

    await render(hbs`{{repo-actions userHasRestartPermissionForRepo=true canOwnerBuild=true job=this.job}}`);
  ...
  test('it does not show restart button if owner cannot build', async function (assert) {
    ...
    await render(hbs`{{repo-actions userHasRestartPermissionForRepo=true canOwnerBuild=false job=this.job}}`);
    assert.dom('button[aria-label="Restart job"]').doesNotExist();
  });
  ...
  test('it shows prioritize button only if build is not prioritized, it is also not already started and if the org and user are having acces of it', async function (assert) {
    this.set('build', EmberObject.create({  canCancel: true }));
    await render(hbs`{{repo-actions userHasCancelPermissionForRepo=true userHasPullPermissionForRepo=true userHasPushPermissionForRepo=true build=this.build canPrioritize=true}}`);
  ...
  test('Prioritize button will be disabled in case user is not having push permission, ', async function (assert) {
    this.set('build', EmberObject.create({  canCancel: true }));
    await render(hbs`{{repo-actions userHasCancelPermissionForRepo=true userHasPullPermissionForRepo=true userHasPushPermissionForRepo=false build=this.build canPrioritize=true}}`);

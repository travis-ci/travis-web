...
module('Acceptance | layouts/default', function (hooks) {
  ...
  test('header layout when authenticated', async function (assert) {
    ...
    assert.equal(defaultHeader.navigationLinks[0].title, 'Travis CI Insights');
    assert.equal(defaultHeader.navigationLinks[1].title, 'Dashboard');
    assert.equal(defaultHeader.navigationLinks[2].title, 'Changelog', 'Shows link to changelog');
    assert.equal(defaultHeader.navigationLinks[3].title, 'Documentation', 'Shows link to documentation');

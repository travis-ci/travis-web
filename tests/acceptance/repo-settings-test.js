...
test('view settings', function(assert) {
  ...
  andThen(function() {
    ...
    assert.ok(settingsPage.crons(0).nextEnqueuing.indexOf('Every') === 0, 'expected cron next enqueuing to match');
    ...
    assert.ok(settingsPage.crons(1).nextEnqueuing.indexOf('Every') === 0, 'expected cron next enqueuing to match');

...
module('Acceptance | home/with repositories', function (hooks) {
  ...
  test('Pusher events change the main display', async function (assert) {
    ...
    this.owner.application.pusher.receive('job:created', generatePusherPayload(job));
    this.owner.application.pusher.receive('build:created', {
    ...
    this.owner.application.pusher.receive('job:queued', generatePusherPayload(job, { state: 'queued' }));
    this.owner.application.pusher.receive('job:received', generatePusherPayload(job, { state: 'received' }));
    ...
    this.owner.application.pusher.receive('build:started', {
    ...
    this.owner.application.pusher.receive('job:started', generatePusherPayload(job, { state: 'started' }));

    this.owner.application.pusher.receive('job:log', {
      id: job.id,
      number: 1,
      final: false,

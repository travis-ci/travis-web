...
module('Acceptance | jobs/debug', function (hooks) {
  ...
  test('debugging job when the current user is not confirmed is not allowed', async function (assert) {
    enableFeature('debugBuilds');

    const currentUser = this.server.create('user', { confirmed_at: null });
    this.server.create('allowance', {subscription_type: 1});
    this.server.create('user', {login: 'travis-ci'});
    this.server.create('allowance', {subscription_type: 1});
    signInUser(currentUser);
    ...
    assert.dom('.action-button--debug').doesNotExist();
  });
});

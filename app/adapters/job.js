...
export default V3Adapter.extend({
  ...
  findMany(store, modelClass, ids, snapshots) {
    ...
    if (build) {
      return this.ajax(`${this.buildURL()}/build/${build.id}/jobs?include=job.commit,job.build`);
    } else {
      return this.ajax(`${this.buildURL()}/job/${snapshots[0].id}?include=job.commit,job.build`);
    }
  }
});

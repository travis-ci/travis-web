...
export default Component.extend({
  ...
  isQueryOutputFilled: gt('probeOutput.length', 0),
  isSaveAllowed: and('isQueryValid', 'isQueryFilled', 'isQueryOutputFilled'),
  probeOutput: '',
      ...
      yield this.api.post('/insights_sandbox/run_query',
        ...
        if (data.success) {
          if (data.hasOwnProperty('positive_results')) {
            this.set(
              'probeOutput',
              `> Array of elements that match the query:<br/>${data.positive_results ? data.positive_results.join('<br/>') : ''}`
            );
          }
          if (data.hasOwnProperty('negative_results')) {
            const result = data.negative_results ? data.negative_results.join('<br/>') : '';
            this.set(
              'probeOutput',
              `${this.probeOutput}<br/><br/>> Array of elements that do not match the query:<br/>${result}`
            );
          }

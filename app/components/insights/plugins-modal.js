...
export default Component.extend({
  ...
  save: task(function* () {
    if (!this.isTestPassed || !this.isFormValid) return;

    try {
      yield this.store.createRecord('insights-plugin', {
        name: this.pluginName,
        publicId: this.publicId,
        privateKey: this.encryptInputValueForAuthentication(this.privateKey),
        pluginType: this.selectedPlugin.id,
        accountName: this.accountName,
        appKey: this.appKey.length > 0 ? this.encryptInputValueForAuthentication(this.appKey) : this.appKey,
        keyHash: this.encryptionKeyHash,
        domain: this.domain,
      ...
      this.reloadPlugins();
      this.onClose();
      this.flashes.success("User Plugin was successfully created! We're running a scan now, "
        + "so it should update on your <a href='/insights/notifications'>Notifications</a> within the next five minutes.");
    } catch (error) {
      this.clearForm(true);
      this.onClose();
    ...
    try {
      const res = yield this.api.post('/insights_plugins/authenticate_key',  {
        data: {
          ...
          app_key: this.appKey.length > 0 ? this.encryptInputValueForAuthentication(this.appKey) : this.appKey,
          domain: this.domain,
  ...
  clearForm: function (clearName) {
    ...
    this.set('appKey', '');
    this.set('domain', '');
    this.set('useForSubplugins', false);
    this.set('selectedPlugin', newInsights.pluginTypes[0]);
  },

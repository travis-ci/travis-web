...
(this, function(moment, $) {
    var DateRangePicker = function(element, options, cb) {
        ...
        this.applyButtonClasses = 'btn-primary active-button';
        this.cancelButtonClasses = 'btn-default cancel-button';
        ...
        if (typeof options.template !== 'string' && !(options.template instanceof $))
            ...
            '<div class="daterangepicker">' +
                ...
                '<div class="drp-buttons">' +
                    ...
                    '<button class="active-button applyBtn" disabled="disabled" type="button"></button> ' +
                '</div>' +
            '</div>';
    ...
    DateRangePicker.prototype = {
        ...
        renderCalendar: function(side) {
            ...
            var curDate = moment([year, month]);

            var col, row;
            for (var i = 0, col = 0, row = 0; i < 12; i++, col++, curDate = moment(curDate).add(1, 'month')) {
                if (i > 0 && col % 3 === 0) {
                    col = 0;
                    row++;
                }
                calendar[row][col] = curDate.clone().format('YYYY-MM');
                if (this.minDate && calendar[row][col].format('YYYY-MM-DD') == this.minDate.format('YYYY-MM-DD') && moment(calendar[row][col]).isBefore(this.minDate) && side == 'left') {
                    calendar[row][col] = this.minDate.clone().format('YYYY-MM');
                }

                if (this.maxDate && calendar[row][col].format('YYYY-MM-DD') == this.maxDate.format('YYYY-MM-DD') && moment(calendar[row][col]).isAfter(this.maxDate) && side == 'right') {
                    calendar[row][col] = this.maxDate.clone().format('YYYY-MM');
                }
            ...
            for (var row = 0; row < 4; row++) {
                ...
                for (var col = 0; col < 3; col++) {
                    ...
                    if (calendar[row][col]== moment(new Date()).format('YYYY-MM'))
                        classes.push('today');

                    //highlight the currently selected start date

                    if (calendar[row][col] == this.startDate.format('YYYY-MM'))
                        classes.push('active', 'start-date');

                    //highlight the currently selected end date
                    if (this.endDate != null && calendar[row][col] == this.endDate.format('YYYY-MM'))
                        classes.push('active', 'end-date');

                    //highlight dates in-between the selected dates
                    if (this.endDate != null && moment(calendar[row][col]).isAfter(this.startDate) && moment(calendar[row][col]).isBefore(this.endDate))
                        classes.push('in-range');
                    var valueDiff =  moment(new Date(this.startDate)).diff(new Date(calendar[row][col]), 'months', true);
                    if(this.endDate != null && this.startDate!=null && moment(calendar[row][col]).isBefore(this.startDate))
                    {
                        if(valueDiff<=monthDifference)
                        {
                ...
                if(moment(calendar[row][col]).isAfter(moment().endOf('day')))
                {
                    ...
                    html += '<td class="' + cname.replace(/^\s+|\s+$/g, '') + '" data-title="' + 'r' + row + 'c' + col + '">' + this.locale.monthNames[3*row+col] + '</td>';
        ...
        hoverDate: function(e) {
            ...
            var startDate = this.startDate.format('YYYY-MM');
            if (!this.endDate) {
        ...
        clickDate: function(e) {
            ...
            if(this.singleClick)
            {
                ...
                this.setEndDate(this.startDate.format('YYYY-MM'));
            } else { // picking end
                this.setEndDate(date);
            }
            if(this.singleClick)
